 ...
 * Last modified  : 2019-03-21 13:15:03
 */
...
const { appid, appsecret, mchid, mchkey, wxurl_custompay } = payconf
...
const unifiedorder = async (ctx, next) => {
    ...
    let { fee, beizhu } = ctx.request.body
    console.log(`fee:${fee},beizhu:${beizhu}` );
    // 校验fee参数
    if( !fee ){
        return ctx.body = {
            code:"CUSTOMPAY_NO_PARAM_FEE_INVALID",
            success:false,
            data:null,
            msg:"金额错误，请输入大于0的正确金额"
        }
    }
    try {
        var total_price = fee * 10 * 10 ;// 单位是分
        console.log(`totalprice:${total_price}`);
        
        // STEP3 组装数据，签名
        let orderid = 'custom_pay' + generateOrderId(); 
        ...
        let notify_url = wxurl_custompay;
                ...
                request({ url: url, method: 'POST', body: formData}, function (err, response, body) {
                    ...
                    if (!err && response.statusCode == 200) {
                        xmlreader.read(body.toString("utf-8"), async function (errors, response) {
                            ...
                            let save = await knex('custompay').insert({
                                openid,
                                orderid,
                                createtime,
                                _createtime,
